...
export function SitesView() {
  ...
  const renderMediaSiteCard = (site: MediaSite, index: number) => {
    ...
    return (
      <Card 
        key={site.id} 
        className={`group hover:shadow-md hover:bg-muted transition-all duration-300 cursor-pointer overflow-hidden rounded-none ${index > 0 ? '-mt-px' : ''}`}
        style={{ animationDelay: `${index * 50}ms` }}
        onClick={() => toggleExpand(site.id)}
      >
        <CardContent className="!p-0">
  ...
  return (
    <div className="animate-fade-in bg-white min-h-[calc(100vh-56px)] lg:min-h-screen -m-4 lg:-m-8 p-4 lg:p-8">
      <div className="max-w-[980px] mx-auto space-y-2 md:space-y-8">
      {/* Header */}
