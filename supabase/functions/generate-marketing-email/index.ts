...
serve(async (req) => {
  ...
  try {
    ...
    if (edit_instructions && previous_html) {
      systemPrompt = `You are an email marketing expert for Stankevicius MGM (stankevicius.co.uk). You edit existing HTML marketing emails based on feedback. Return ONLY the full updated HTML email body — no markdown, no code fences, no explanation. The email must be professional, mobile-responsive, and use inline CSS styles. Keep the Stankevicius branding.`;
