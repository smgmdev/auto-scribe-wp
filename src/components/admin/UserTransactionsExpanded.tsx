  ...
  return (
    <div className="bg-muted/30 border-t">
      <Tabs value={activeType} onValueChange={setActiveType}>
        <TabsList className="flex flex-wrap justify-start h-auto gap-1 bg-transparent p-0 border-y border-border">
          {transactionTypes.map(type => {
            ...
            return (
              <TabsTrigger
                key={type.key}
                value={type.key}
                className="data-[state=active]:bg-foreground data-[state=active]:text-background px-3 py-1 text-xs !rounded-none"
              >
                {type.label} ({count})
              </TabsTrigger>
            );
