...
DECLARE
  _profile RECORD;
  _stale_threshold interval := interval '35 minutes';
...
BEGIN
  ...
  -- If last_online_at is older than 35 minutes, the session is stale — auto-clear it
  ...
  -- If the auth server has no sessions (e.g. technical logout, env rebuild),
  -- the profile session is orphaned — clear it.
  SELECT EXISTS (
