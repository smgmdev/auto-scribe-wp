    ...
    const fetchCountryMissiles = async () => {
      ...
      if (data) {
        ...
        const filtered = data.filter(m => {
          ...
          const analyticalPatterns = ['depletes', 'depleted', 'reserves', 'stockpile', 'running out', 'running low', 'cost of', 'costs of', 'spending on', 'impact on', 'effect on', 'consequences of', 'toll of', 'analysis:', 'opinion:', 'editorial:', 'commentary:', 'lessons from', 'lessons of', 'learns a', 'learned a', 'lesson on', 'lesson in', 'lesson from', 'tough lesson', 'hard lesson', 'warns about', 'warns of', 'prepares for', 'plans to', 'threatens to', 'vows to', 'vows no', 'vows not', 'vows never', 'vows revenge', 'vows retaliation', 'vowed', 'pledges to', 'pledged to', 'promises to', 'promised to', 'arms race', 'war economy', 'war fatigue', 'no retreat', 'will not back down', 'doubles down', 'stands firm', 'reaffirms', 'signals intent', 'is unfolding', 'war is unfolding', 'on day ', 'and more ...', 'and more…', 'what we know', 'what happened', 'here\'s what', 'everything we know', 'wrap up', 'roundup', 'round-up', 'recap', 'in summary', 'day-by-day', 'timeline of', 'how the', 'how iran', 'how russia', 'how israel'];
          if (analyticalPatterns.some(p => titleLower.includes(p))) return false;
          // Filter out compound/roundup headlines with semicolons listing multiple locations — these are summaries, not single confirmed attacks
          if (titleLower.includes(';') && ['and more', 'unfolding', 'day '].some(p => titleLower.includes(p))) return false;
          // Filter out personal/travel/lifestyle/celebrity stories
          const personalPatterns = ['techie', 'returns to', 'shares real', 'traveler', 'traveller', 'tourist', 'passenger', 'km away', 'miles away', 'close call', 'narrow escape', 'viral video', 'goes viral', 'bengaluru', 'bangalore', 'delhi news', 'via muscat', 'via dubai', 'via doha', 'speaks out', 'tells story', 'recounts', 'real housewives', 'celebrity', 'celebrities', 'tv star', 'tv personality', 'influencer', 'reality tv', 'reality show', 'separated from', 'reunited with', 'fans react', 'fans worry', 'heartbreaking', 'emotional reunion', 'stuck in', 'find a way back', 'way back to', 'describes life under', 'describes life in', 'american stuck'];
