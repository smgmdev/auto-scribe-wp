  ...
  useEffect(() => {
    const fetchLatestArticles = async () => {
      const { data, error } = await supabase
        .from('articles')
        .select('id, title, created_at, wp_link, published_to_name, published_to_favicon')
        .eq('status', 'published')
        ...
        <CarouselContent className="-ml-4">
          {articles.map((article) => (
            <CarouselItem key={article.id} className="pl-4 md:basis-1/3">
              ...
              >
                <div className="rounded-2xl bg-card border border-border p-6 h-full hover:border-accent/50 hover:shadow-lg transition-all duration-200 flex flex-col">
                  {/* Site info */}
                  ...
                  {/* Title */}
                  <h3 className="font-semibold text-foreground text-lg leading-snug line-clamp-3 flex-1 group-hover:text-accent transition-colors">
                    {article.title}
                  </h3>
